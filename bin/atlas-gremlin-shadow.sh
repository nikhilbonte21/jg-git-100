...
cd $(dirname $0)
DIR=`pwd`
#ATLAS_WEBAPP_DIR='/Users/suman/atlan/atlas-metastore/webapp/target/atlas-webapp-3.0.0-SNAPSHOT/'
...
JAVA_OPTIONS="${JAVA_OPTIONS} -Duser.working_dir=${USER_DIR} -Dtinkerpop.ext=${USER_EXT_DIR:-${SYSTEM_EXT_DIR}} -Dlog4j.configuration=conf/log4j-console.properties -Dgremlin.log4j.level=$GREMLIN_LOG_LEVEL -javaagent:$LIB/jamm-0.3.3.jar -Dgremlin.io.kryoShimService=org.janusgraph.hadoop.serialize.JanusGraphKryoShimService"
...
exec $JAVA $JAVA_OPTIONS $MAIN_CLASS -i scripts/startup-shadow.groovy "$@"
